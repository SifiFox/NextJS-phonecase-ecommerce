...
const DesignConfigurator = ({configId, imageUrl, imageDimensions}: DesignConfiguratorProps) => {
    ...
    const {mutate: saveConfig} = useMutation({
        ...
        onError: () => {
            toast({
                ...
                description: "There was error. Try again",
                variant: "destructive"
            })
        },
        onSuccess: () => {
    ...
    async function saveConfiguration() {
        try {
            ...
            toast({
                title: "Something went wrong",
                description: "There was a problem saving your config, try again",
                variant: 'destructive'
            })
        }
    }
    ...
    return (
        <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20 gap-6 lg:gap-0">
            <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center
             justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center
                                ...
                                <RadioGroup value={options.color} onChange={(value) => {
                                    ...
                                    <div className="mt-3 flex items-center space-x-3">
                                        {COLORS.map((color) => (
                                            ...
                                            >
                                                <span
                                                    className={cn(`bg-${color.tw}`, 'w-5 h-5 rounded-full border-black border-opacity-10')}/>
                                            </RadioGroup.Option>
                                        ))}
                                    </div>
                                </RadioGroup>
                                            ...
                                            {MODELS.options.map((model) => (
                                                <DropdownMenuItem
                                                    ...
                                                    className={cn('flex text-sm gap-1 items-center p-1.5 cursor-default hover:bg-zinc-100', {
                                                        ...
                                                        "cursor-pointer": model.label !== options.model.label,
                                                    })}
                                                    onClick={() => {
                                        ...
                                        <div className="mt-3 space-y-4">
                                            {selectableOptions.map((option) => (
                                                ...
                                                >
                                                        ...
                                                        <span className="flex flex-col text-sm">
                                                            ...
                                                            {option.description && (
                                                                <RadioGroup.Description
                                                                    as="span"
                                                                    className="text-gray-500 block sm:inline text-xs"
                                                                >
                                                    ...
                                                    >
                                                        <span
                                                            className="font-medium text-gray-900">{formatPrice(option.price / 100)}</span>
                                                    </RadioGroup.Description>
                                                </RadioGroup.Option>
                                            ))}
                                        </div>
